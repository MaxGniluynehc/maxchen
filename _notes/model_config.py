...
# determine which device is used for computation
...
################################################################################################
# define functions to load data and train the model

def load_and_cache_examples(task, tokenizer, evaluate=False): # evaluate = True if we are testing the model
    processor = processors[task]()
    output_mode = args['output_mode']

    mode = 'dev' if evaluate else 'train' # evaluate determined whether to train or test the model.
...
def evaluate(model, tokenizer, prefix=""):
    ...
    preds = None
    out_label_ids = None
    for batch in tqdm_notebook(eval_dataloader, desc="Evaluating"): # get the batch from the dataloader
        model.eval()
        batch = tuple(t.to(device) for t in batch)
...
# Freeze the bert layer and only train the classifier layer
if args['do_train']:
    train_dataset = load_and_cache_examples(task, tokenizer)
    model = model_class.from_pretrained(args['model_name'])
    model.to(device)
    for param in model.bert.parameters(): # freeze all the bert layers
        param.requires_grad = False
    global_step, tr_loss = train_with_early_stopping(train_dataset, model, tokenizer)
    logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)

    # Then unfreeze bert layers and freeze the classifier layer
